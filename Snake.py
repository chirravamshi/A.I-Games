...
HEIGHT=20#num of steps vertically from wall to wall of screen
WIDTH=20#num of steps horizontally from wall to wall of screen
...
APPLE_SHAPE='circle'
APPLE_COLOR='green'
class Snake(gym.Env):#put gym.Env
    def __init__(self,human=False,env_info={'state_space':None}):
        ...
        ##GAME CREATION WITH TURTLE#screen background
        self.win=turtle.Screen()
        self.win.title(GAME_TITLE)
        self.win.bgcolor(BG_COLOR)
        self.win.tracer(0)
        self.win.setup(width=PIXEL_W+35,height=PIXEL_H+35)
        #snake
        ...
        # snakebody,addfirstelemen(forlocationofsnakehead)
        self.snake_body=[]
        self.add_to_body()
        # apple
        ...
        self.score.goto(-100,180)
        self.score.write(f"Total:{self.total}Highest:{self.maximum}",align='Center',font=('Courier',15,'bold'))
        #control
        self.win.listen()
    ...
    def run_game(self):
        ...
        if self.human:
            time.sleep(SLEEP)
            state=self.get_state()#calling Get_state
    #AI agent
